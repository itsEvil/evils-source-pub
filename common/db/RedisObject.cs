...
public abstract class RedisObject
{
    ...
    protected T GetValue<T>(RedisValue key, T def = default)
    {
        ...
        if (typeof(T) == typeof(ItemModel[]))
        {
            ...
            var data = JsonSerializer.Deserialize<ItemModel[]>(itemString, SourceGenerationContext.Default.ItemModelArray);
            return (T)(object)data;
        }

        if (typeof(T) == typeof(PotionModel[]))
        { 
            var potionString = Encoding.UTF8.GetString(val.Key);
            var data = JsonSerializer.Deserialize<PotionModel[]>(potionString, SourceGenerationContext.Default.PotionModelArray);
            return (T)(object)data;
        }
    ...
    protected void SetValue<T>(RedisValue key, T val)
    {
        ...
        {
            var arr = (ItemModel[])(object)val;
            buff = Encoding.UTF8.GetBytes(arr.ToJson());
        }
        else if(typeof(T) == typeof(PotionModel[]))
        {
            var arr = (PotionModel[])(object)val;
            buff = Encoding.UTF8.GetBytes(arr.ToJson());
        }
        else
            throw new NotSupportedException();
